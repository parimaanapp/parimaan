...
# here — a device push credential, W22 registers it via its own mutation, no
...
# present at all — curated (W15/W16 seeder) and ai (a future
...
type Menu {
  ...
  items: [MenuItem!]!
  # W14 (E2E_MVP_PLAN.md §20.2.4, D4). The frozen meal configuration this week
  # was planned under — { mealsEnabled, mealStructure, snapshotAt }. Every
  # cap/slot read for this week (addMenuItem, autoFillWeek, autoFillPreview,
  # and the mobile weekly grid) reads THIS, not the household's live settings;
  # only createMenu reads live settings, and that read IS the snapshot. Adding
  # a field to a type changes no mutation's return type, so no @aws_subscribe
  # list is affected — checked, not assumed.
  mealConfigSnapshot: AWSJSON!
}
...
type Subscription {
  ...
  # W20 item, not solved here. `haveIt` itself was widened to `ShoppingList!`
  ...
  # "subscribe-time-only re-authorization" gap (§14, flagged W11/W22) for
...
CREATE TABLE menus (
  ...
  -- Added W14 (E2E_MVP_PLAN.md §20.2.1, Q17). Frozen copy of the household's
  -- meal configuration as it stood when this week's plan was created:
  --   { "mealsEnabled": ["breakfast","lunch","dinner"],
  --     "mealStructure": { "lunch": {...}, "dinner": {...} },
  --     "snapshotAt": "2026-09-14T00:00:00Z" }
  -- Written exactly once, by `createMenu`, and NEVER updated — that absence is
  -- the guarantee, not a convention: a later `updateHouseholdSettings` cannot
  -- reshape a week already in progress. One combined column rather than two
  -- (`meals_enabled_snapshot` + `meal_structure_snapshot`) because the halves
  -- are only meaningful together and a single column makes "write both or
  -- neither" structural. Read defensively, exactly like
  -- `household_settings.meal_structure` (no shape guarantee beyond valid JSON;
  -- a missing/malformed entry caps at 0, never at a "reasonable default").
  meal_config_snapshot JSONB NOT NULL,
...
**Client-only reference data (no schema, no resolver):** three small modules under `mobile/lib/features/pantry/domain/` hold vocabulary rather than logic — `pantry_unit.dart` and `pantry_category.dart` (hand-mirrored twins of `api/src/domain/pantryUnits.ts` / `pantryCategories.ts`, whose own docs describe them as *"a guess from `PRD.md` §7.1, not a locked taxonomy — meant to grow via edits here, never a migration"*), and, added W14, **`curated_pantry_items.dart`** — a per-category, hand-ordered, most-likely-first list of common item names powering the pantry category picker (E2E_MVP_PLAN.md §20.2.5, Q21). It is **client-only by design: no table, no resolver, no query.** It ships as a Dart constant, is allowed to be incomplete and wrong at first, carries names only (never quantities or units — those come from the user), and is corrected by editing the file, exactly as its two neighbours are. Its multi-select commit path adds nothing to the API surface either: it calls the already-shipped `bulkAddPantryItems` mutation (W5, ≤50 items, single transaction), becoming that mutation's first real client — and inherits its deliberate absence from `onPantryChanged`'s `@aws_subscribe` list (§6.1's own note; a list-shaped payload cannot fan out to a single-`PantryItem` subscription), so a multi-select add reaches other members on their next refetch rather than as a push.
...
> **Amended 2026-09-07** (Phase 3b planning, W13–W14 — `E2E_MVP_PLAN.md` §10 Q16–Q21, §19–§20): three changes to this document, all ahead of implementation rather than after it, plus one renumbering.
> - **`menus` gains `meal_config_snapshot JSONB NOT NULL`** (§7.1 DDL above) and **`Menu` gains `mealConfigSnapshot: AWSJSON!`** (§6.1). A household's meal configuration (`mealsEnabled` + `mealStructure`) is durable, but a week already in progress must not be reshaped by a later settings edit — so the configuration is frozen onto the `menus` row at creation time and read from there by `addMenuItem`, `autoFillWeek`, `autoFillPreview` and the mobile weekly grid. Only `createMenu` reads live `household_settings`, and that read *is* the snapshot. This is `MenuItem.slotRole`'s already-locked rule (*"a menu is a snapshot of what was planned, not a live view"*) extended from one column to the config that shapes the whole grid; no cap-enforcement function changes, only the source it is handed. Adding a field to a type changes no mutation's return type, so no `@aws_subscribe` list is affected.
> - **A new client-only reference module, `curated_pantry_items.dart`** (§9.1) — per-category curated pantry suggestions, hardcoded in Dart alongside `pantry_unit.dart`/`pantry_category.dart`. **No new table, no new resolver, no new query**; its commit path reuses the already-shipped `bulkAddPantryItems`. Recorded here only so a future reader does not go looking for the table.
> - **Week numbers in this document shifted +2 from W13 onward** (W13→W15, W14→W16, W15→W17, W17→W19, W18→W20, W19→W21, W20→W22), because two live-feedback weeks were inserted after W12. Content and order of every renumbered week are unchanged; see `E2E_MVP_PLAN.md` §4 and §10 Q16.

...
**Cleanup status — INCOMPLETE, named rather than silently left.** Both throwaway households remain in dev Aurora, undeletable via any existing resolver until the FK gap above is fixed:
- `98b557a1-cbc5-4558-9526-af36203fa2be` ("W11S4 RDS Spike Household")
- `b7d79697-d93d-4910-bc25-cbdbd30a77db` ("W11S4 RDS Spike Household 2")

Both are synthetic-identity-only (no real user, no pre-existing data touched) and clearly named/tagged for identification. No pre-existing household, user, recipe, or menu was read or modified by this pass. Delete both once the FK fix ships.
...
**§17.8's two orphaned households remain undeletable by this pass, for a reason worth recording as its own process gap.** `98b557a1-cbc5-4558-9526-af36203fa2be` and `b7d79697-d93d-4910-bc25-cbdbd30a77db` were created by S4's spike script under a synthetic identity whose `sub` was never recorded in §17.8's write-up — only the household id and name were. `deleteHousehold` requires the caller to be that household's `primary` member, and `Query.household` itself requires membership just to look anything up — with no record of the original synthetic `sub`, there is no way for this pass (or any future one) to reconstruct a valid caller identity for either household without guessing a UUID, which this pass will not do. Per this task's own safety scope ("only touch throwaway data you create yourself"), these two are correctly left untouched rather than worked around. **Recommendation, recorded in RUNBOOK.md §2 below: any future verification pass must record the synthetic `sub`(s) used, not just the resulting household id/name, specifically so a later cleanup pass (this one, or the FK fix itself landing after the spike) can actually act as that identity.** Both households are still synthetic-identity-only, clearly named, and touch no real user/recipe/menu — an accepted, named leftover, not a silent one.

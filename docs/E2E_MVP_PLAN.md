...
- [x] The Gemini API key lives in Secrets Manager, is fetched at cold start and cached per container following the existing `APP_ROLE_SECRET_ARN` pattern, is **never** in CDK source, an env var value, a log line, or a client-facing error (asserted by `geminiClient.test.ts`'s header-not-URL and no-internals-in-message tests) — **partial:** `secretsmanager:GetSecretValue` ARN-scoping is implemented in `api-stack.ts`'s `createAiAndNetResolvers` and will apply the moment a real entry exists, but `AI_RESOLVERS` is still empty as of S2 (no SDL field yet), so there is no live Lambda in the synthesized template yet for a CDK-template-level test to assert the grant against — that assertion lands with S3 (S2/S3)
...
- [ ] **The rate limit is consumed exactly once per user-initiated call regardless of internal retries**, asserted by a named test and reviewed as a cost control (D7/D8, §13.2.9) — `invokeModel` is deliberately rate-limiter-agnostic (documented in its own doc comment: the property falls out of "caller checks once, calls `invokeModel` once" by construction), so the actual assertion needs a real resolver and lands with S3
- [x] Unknown enum values from the model degrade one field with a warning rather than failing the parse; structural and bounds violations still fail hard — **the generic mechanism** (schema-level `.catch()` support, bounds-violation rejection) is proven at `invokeModel`'s level (S2); the domain-specific `RecipeDraft.warnings` implementation is S3's job (D4, S2/S3)

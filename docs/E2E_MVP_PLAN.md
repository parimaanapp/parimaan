...
| **W14** | Meal-config snapshot per menu + pantry curated picker | (43/50 — no new wireframe screen, §20.2.6) | `menus.meal_config_snapshot` migration; every generation/add read path switches to the menu's own snapshot; client-side curated per-category item list; first mobile caller of `bulkAddPantryItems` | A settings edit mid-week never changes the week already in progress; ticking curated items writes them in one `bulkAddPantryItems` call |

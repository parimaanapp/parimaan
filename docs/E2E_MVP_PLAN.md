...
| **W13** | First-run welcome + meal-instance grouping + `mealsEnabled` editable | **Welcome · what first?** (43/50) | `updateHouseholdSettings` partial-patch fix for `mealStructure` (the Dinner-clobber bug, §19.2.4); Settings → "Meals to plan" edit route (all four meal types) | Welcome screen shows only for a household with no real activity; Weekly plan renders four meal-instance headers; editing Lunch structure leaves Dinner's counts intact |

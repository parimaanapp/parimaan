...
describe('menus and menu_items, once migrated', () => {
  ...
  // this file must supply one — a fixed placeholder here since this file's
  // own tests are about the `menus`/`menu_items` DDL and RLS shipped in
  // `1788100000000_menus.ts`, not the snapshot's content (that lives in
  // `migrations.menuConfigSnapshot.test.ts`).
    ...
    try {
      await expect(
        asOutsider.query(
          ...
          [householdA.id, placeholderSnapshot],
        ),
  ...
  it('lets the real parimaan_app role do full CRUD on menus and menu_items (the grant this migration adds)', async () => {
    ...
    try {
      ...
      const insertedMenu = await appClient.query<{ id: string }>(
        ...
        [household.id, placeholderSnapshot],
      );

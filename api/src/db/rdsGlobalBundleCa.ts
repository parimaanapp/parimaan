/* eslint-disable max-lines -- an embedded PEM certificate bundle, not code; the line count is AWS's, not this file's. */
/**
 * AWS's global RDS/Aurora TLS CA bundle, embedded verbatim.
 *
 * `pool.ts` connects with `ssl: { rejectUnauthorized: true }` (server
 * identity verification on, deliberately — the alternative,
 * `rejectUnauthorized: false`, accepts any certificate at all and defeats
 * the point of TLS). Node's default trusted-root bundle does not include
 * Amazon's RDS-issued CA, so without this, every connection attempt fails
 * with `unable to get local issuer certificate` — caught only by a real
 * connection attempt against the live cluster, since nothing synth-time
 * or unit-tested actually negotiates TLS.
 *
 * The global bundle (not the `ap-south-1`-only one) is used deliberately:
 * it verifies a cluster in any region, so a future cross-region move or
 * read replica does not silently need this constant updated too.
 *
 * Source: https://truststore.pki.rds.amazonaws.com/global/global-bundle.pem
 * — AWS's own documented, stable download URL for this bundle
 * (https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/UsingWithRDS.SSL.html).
 * Embedded as a source constant, not fetched at runtime or Lambda-bundled
 * as a separate asset file, so it needs no extra CDK bundling step across
 * the 9 separate resolver Lambda entry points that import it — esbuild
 * includes it automatically, the same as any other imported module.
 */
export const RDS_GLOBAL_BUNDLE_CA = `
-----BEGIN CERTIFICATE-----
